...
`code == 0` 为成功；非 0 或 HTTP 4xx/5xx 为失败，`message` 为原因。基址示例：`https://hk3.geek2api.com`，前缀 `/api/v1`。
...
- **token 存储**：refresh_token 是长期凭证，优先使用系统密钥库（macOS Keychain / Windows Credential Manager / libsecret），不要明文落配置文件；日志里对 token 打码。XAiWebHook 的 `browser.session_cache_enabled` 默认关闭；显式启用后会把 Playwright storageState（含 Cookie/localStorage/refresh token）写入受限缓存目录，必须保护插件配置目录，安全要求较高时应保持关闭。
...
BASE = "https://hk3.geek2api.com"          # 你的站点
POLL_INTERVAL = 3                           # 秒
