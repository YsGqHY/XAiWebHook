...
send_webpage_screenshot：按 `steps` 执行网页访问。`auth.cli_bridge` 支持系统浏览器授权、一次性 token 轮询、profile 用户补全、refresh token 轮换和四项 localStorage 登录态；兼容模式仍支持 `auth.login` 账号密码/2FA、AccessToken 请求头、Cookie 与 `auth.bootstrap`。命名 `session_key` 可配合 `browser.session_cache_enabled` 持久化 Cookie/localStorage。最后截取 `screenshot.selector` 元素并发送；`screenshot.hide_selectors` 可在截图瞬间隐藏固定/悬浮元素，`screenshot.timeout_ms` 控制截图超时，`screenshot.font_wait_timeout_ms` 提供可降级的有界字体等待，`screenshot.retry` 控制可重试失败的最大次数与间隔。可选参数含 `session_key`、`group_id`、`friend_id`、`pending_message`、`failure_message`、`message`。
