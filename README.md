...
`screenshot.timeout_ms` 控制目标元素等待与截图操作超时，默认回退 `browser.timeout_ms`。`screenshot.font_wait_timeout_ms` 控制截图前等待网页字体完成加载的最长时间，默认 `3000` 毫秒且不会超过 `screenshot.timeout_ms`，设为 `0` 可直接跳过等待。字体 CDN、代理或网络请求长期挂起时，插件会记录调试日志并使用当前浏览器已渲染的回退字体继续截图，不再因 Playwright 的字体等待耗尽整个截图超时。

`screenshot.hide_selectors` 是截图瞬间临时隐藏的 CSS selector 列表，最多 32 项；插件为命中元素注入 `visibility: hidden !important`，截图完成后由 Playwright 自动移除样式，不修改页面状态，也不会引发布局重排。该字段只接受 CSS selector，不接受 XPath。Geek2Api 的 AppHeader 为 `header.sticky`，长 `<main>` 元素截图发生滚动时会覆盖顶部内容，因此默认示例将它加入隐藏列表。

`screenshot.retry` 控制失败重试，未配置时默认关闭；配置区块存在时 `enabled` 默认 `true`。`max_retries` 表示首次失败后的最大额外重试次数，默认 `2`、范围 `0-10`，因此 `max_retries: 2` 最多执行 3 次完整的 `steps + screenshot`。`delay_ms` 是固定重试间隔，默认 `1000` 毫秒、范围 `0-60000`。每次重试创建新 Page，但继续复用同名 session 的 Cookie/token；pending 消息只发送一次，failure 消息只在所有尝试均失败后发送。仅浏览器 step/截图的超时与明确网络瞬断会重试，认证失败、配置错误、非法 selector 和截图大小超限不会重试。启用动作组 `single_flight` 时锁会覆盖整个重试周期；最坏耗时会随总尝试次数累加，配置长超时时应谨慎设置 `max_retries`。

相同 `session_key` 会在内存中复用 BrowserContext 和 token pair，每次执行仍使用独立 Page。启用 `browser.session_cache_enabled` 时，缓存文件写入 `<插件配置目录>/<session_cache_dir>/<session_key 的 SHA-256>.json`，不会把原始 session key 作为路径；旧 `auth.login` 文件模式的邮箱和密码仍只来自 YAML 或环境变量，缓存仅保存浏览器 storageState 与 token 类型。
...
- `send_webpage_screenshot`：按 `steps` 执行网页访问。`auth.cli_bridge` 支持系统浏览器授权、一次性 token 轮询、profile 用户补全、refresh token 轮换和四项 localStorage 登录态；兼容模式仍支持 `auth.login` 账号密码/2FA、AccessToken 请求头、Cookie 与 `auth.bootstrap`。`base_urls` 可为同一站点配置多个候选基址，当前 URL 成功时保持使用，任意请求/导航错误时切换到下一条。命名 `session_key` 可配合 `browser.session_cache_enabled` 持久化 Cookie/localStorage。最后截取 `screenshot.selector` 元素并发送；`screenshot.hide_selectors` 可在截图瞬间隐藏固定/悬浮元素，`screenshot.timeout_ms` 控制截图超时，`screenshot.font_wait_timeout_ms` 提供可降级的有界字体等待，`screenshot.retry` 控制可重试失败的最大次数与间隔。可选参数含 `session_key`、`base_urls`、`group_id`、`friend_id`、`pending_message`、`failure_message`、`message`。
